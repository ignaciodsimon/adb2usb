...
// ---------------------------------------------------------------------------------------
// Joe -- Begin of added code

size_t HIDKeyboard::pressKeycode(uint8_t k)
{
    if (k == 0)
    {
        return 0;
    ...
    // Check if they key was already "down", then do nothing
    ...
    // Find an empty spot and set it, then send it
    for (unsigned i = 0; i < HID_KEYBOARD_ROLLOVER; i++)
    {
...
size_t HIDKeyboard::releaseKeycode(uint8_t k)
{    
    if (k == 0)
    {
        return 0;
    ...
    // Find key and remove from the list
    for (unsigned i = 0; i < HID_KEYBOARD_ROLLOVER; i++)
    {
         if (keyReport.keys[i] == k)
         {
